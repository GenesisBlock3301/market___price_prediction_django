...
# 1
def process_csv_display1(request):
    path = str(settings.BASE_DIR) + "/media/Day-k.csv"
    print(">>>>>>>>>>>>>>>>>>>>>>>>", "Working", path)
    ...
    try:
        ...
        Three_Outside_Up_Down = talib.CDL3OUTSIDE(df['open'].values, df['high'].values, df['low'].values,
                                                  df['close'].values)
        Three_Stars_In_The_South = talib.CDL3STARSINSOUTH(df['open'].values, df['high'].values, df['low'].values,
                                                          df['close'].values)
        Three_Advancing_White_Soldiers = talib.CDL3WHITESOLDIERS(df['open'].values, df['high'].values,
                                                                 df['low'].values,
                                                                 df['close'].values)
        ...
        Closing_Marubozu = talib.CDLCLOSINGMARUBOZU(df['open'].values, df['high'].values, df['low'].values,
                                                    df['close'].values)
        Concealing_Baby_Swallow = talib.CDLCONCEALBABYSWALL(df['open'].values, df['high'].values, df['low'].values,
                                                            df['close'].values)
        Counterattack = talib.CDLCOUNTERATTACK(df['open'].values, df['high'].values, df['low'].values,
                                               df['close'].values)
        Dark_Cloud_Cover = talib.CDLDARKCLOUDCOVER(df['open'].values, df['high'].values, df['low'].values,
...
def process_prediction1(request):
    global res
...
# 2
def process_csv_display2(request):
    # df = pd.read_csv('Pattern_Recognised_.csv')
    global dt_;
    df = pd.read_csv(str(settings.BASE_DIR) + "/media/trained/Pattern_Recognised.csv")
    dt_ = df.loc[df['Pattern'] == res]
    # del dt_['pattern_class']
    # del dt_['AI_date']
    data = dt_.to_dict(orient='records')
    return JsonResponse(data, safe=False)


def process_prediction2(request):
    pass


def graph2(request):
    fig = go.Figure(data=[go.Candlestick(x=dt_[dt_.columns[0]],
                                         open=dt_[dt_.columns[1]],
                                         high=dt_[dt_.columns[2]],
                                         low=dt_[dt_.columns[3]],
                                         close=dt_[dt_.columns[4]])])
    fig.update_layout(xaxis_rangeslider_visible=False)
...
def process_csv_display3(request):
    ...
    data = du.to_dict(orient='records')
    return JsonResponse(data, safe=False)
